...
async function gyapuCrawler() {
    const db = 'Ecommerce-Price-Tracker'
    mongoose
  .connect(
        ...
        "mode": "cors"
            ...
            for(let i = 0; i<=11; i++){
                ...
                try {
                 ...
                 let newAllProduct = new AllProduct({
                    _id: result.body.data[i]._id,
                    name: result.body.data[i].name,
                    imageUrl: "https://www.gyapu.com/"+result.body.data[i].image[0].document.path,
                    link: "https://www.gyapu.com/detail/"+result.body.data[i].url_key,
                    price: result.body.data[i].max_sales_price,
                    site: "Gyapu"
                })
                newAllProduct.save((e)=>{
                    if(e){
                        console.error(e);
                    }else{
    ...
    try {
        ...
        "mode": "cors"
            ...
            for(let i = 0; i<=11; i++){
                // console.log(pageNo)
                try {
                console.log(result.body.data.products[i]._id)
                 console.log(result.body.data[i].name);
                 console.log("https://www.gyapu.com/detail/"+result.body.data[i].url_key);
                 console.log("https://www.gyapu.com/"+result.body.data[i].image[0].document.path);
                 console.log(result.body.data[i].max_sales_price);
